...
class ReceiptService {
  ...
  static String generateTextReceipt({
    ...
    final String addressee = (student.parentName != null && student.parentName!.trim().isNotEmpty)
        ? student.parentName!.trim()
        : student.fullName.trim();

    return """
${header}Dear $addressee,

We have successfully received your ${isAdvance ? 'advance ' : ''}payment of *₹${amount.toStringAsFixed(0)}* for *${student.fullName}* (Class: ${student.studentClass}).
...
""";
  ...
  static Future<void> shareToWhatsApp({
    ...
    if (pdfFile != null) {
      try {
        ...
        final shortUrl = await _shortenUrl(signedUrl);
        finalMessage += '\n\n📄 *Official E-Receipt*\nView & Download: $shortUrl\n_Note: This secure link is valid for 7 days._';
      } catch (e) {
  ...
  static Future<String> _shortenUrl(String longUrl) async {
    try {
      ...
      final request = await client.getUrl(Uri.parse('https://tinyurl.com/api-create.php?url=${Uri.encodeComponent(longUrl)}'));
      ...
      debugPrint('URL shortening failed: $e');
    }
